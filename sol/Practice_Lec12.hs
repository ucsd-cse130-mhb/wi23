...
subseqs :: [a] -> [[a]]
subseqs xs = case xs of
               [] -> [[]]
               (y:ys) -> let ysubs = subseqs ys in
                           ysubs ++ consAll y ysubs
...
subseqs' :: [a] -> [[a]]
subseqs' xs = foldr f [[]] xs
  where f x acc = acc ++ consAll x acc
